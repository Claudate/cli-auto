...
//!
//! [INPUT]: Config · clap GitCommands / GitBranchCommands / GitStashCommands / GitTagCommands fields
//! [OUTPUT]: exit code
    ...
    match cmd {
        GitCommands::Status { project } => {
            ...
            if let Some(n) = &v.user_name {
                println!(
                    "\nidentity: {n} <{}>",
                    v.user_email.as_deref().unwrap_or("")
                );
            } else {
                println!("\nidentity: (not set)");
            }
        ...
        GitCommands::Pull {
            ...
            println!("{}", r.message);
            if r.merged {
                println!("files changed: {}", r.files_changed);
            }
            if let Some(o) = &r.output {
                println!("{o}");
            }
        ...
        GitCommands::Tag { cmd } => run_tag(config, cmd),
    }
}
...
fn run_remote(config: &Config, cmd: GitRemoteCommands) -> Result<i32> {
    match cmd {
        ...
        GitRemoteCommands::Add {
            ...
            let reg = normalize_region(&region).ok_or_else(|| {
                anyhow::anyhow!("invalid region: {region} (use domestic|overseas)")
            })?;
...
fn run_stash(_config: &Config, cmd: GitStashCommands) -> Result<i32> {
    match cmd {
        GitStashCommands::List { project } => {
            let proj = resolve_project(project)?;
            let entries = git_svc::stash_list(&proj)?;
            for e in &entries {
                println!("stash@{{{}}}: {}", e.index, e.message);
            }
            Ok(0)
        }
        GitStashCommands::Push { project, message } => {
            let proj = resolve_project(project)?;
            let out = git_svc::stash_push(&proj, message.as_deref())?;
            println!("{out}");
            Ok(0)
        }
        GitStashCommands::Pop { project, index } => {
            let proj = resolve_project(project)?;
            let out = git_svc::stash_pop(&proj, index)?;
            println!("{out}");
            Ok(0)
        }
        GitStashCommands::Apply { project, index } => {
            let proj = resolve_project(project)?;
            let out = git_svc::stash_apply(&proj, index)?;
            println!("{out}");
            Ok(0)
        }
        GitStashCommands::Drop { project, index } => {
            let proj = resolve_project(project)?;
            let out = git_svc::stash_drop(&proj, index)?;
            println!("{out}");
            Ok(0)
        }
        GitStashCommands::Show { project, index } => {
            let proj = resolve_project(project)?;
            let out = git_svc::stash_show(&proj, index)?;
            println!("{out}");
            Ok(0)
        }
    }
...
fn run_tag(_config: &Config, cmd: GitTagCommands) -> Result<i32> {
    match cmd {
        GitTagCommands::List { project } => {
            let proj = resolve_project(project)?;
            let tags = git_svc::list_tags(&proj)?;
            if tags.is_empty() {
                println!("(no tags)");
            }
            for t in &tags {
                if t.message.is_empty() {
                    println!("{:<20} {}", t.name, t.commit);
                } else {
                    println!("{:<20} {}  {}", t.name, t.commit, t.message);
                }
            }
            Ok(0)
        }
        GitTagCommands::Create {
            project,
            name,
            commit,
        } => {
            let proj = resolve_project(project)?;
            let msg = git_svc::create_tag(&proj, &name, commit.as_deref())?;
            println!("{msg}");
            Ok(0)
        }
        GitTagCommands::Annotate {
            project,
            name,
            message,
            commit,
            ...
            let proj = resolve_project(project)?;
            let msg = git_svc::create_annotated_tag(&proj, &name, &message, commit.as_deref())?;
            println!("{msg}");
            Ok(0)
        }
        GitTagCommands::Delete { project, name } => {
            let proj = resolve_project(project)?;
            let msg = git_svc::delete_tag(&proj, &name)?;
            println!("{msg}");
            Ok(0)
        }
        GitTagCommands::Show { project, name } => {
            let proj = resolve_project(project)?;
            let out = git_svc::show_tag(&proj, &name)?;
            println!("{out}");
            Ok(0)
        }
    }
