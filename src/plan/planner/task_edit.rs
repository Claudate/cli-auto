...
/// Reject unknown provider names on confirm edit (includes opt-in `sdk` name).
pub fn validate_provider_name(p: &str) -> Result<String> {
    let p = p.trim().to_ascii_lowercase();
    if p.is_empty() {
        bail!("provider 不能为空");
    }
    let ok = matches!(p.as_str(), "claude" | "codex" | "fake" | "sdk");
    if !ok {
        bail!("不支持的 provider: {p}（可选 claude / codex / fake / sdk）");
    }
    Ok(p)
}
