...
## 严重度（写错会卡死整轮 · 必须遵守）
- **blocking**：功能/验收未落地、红测、编译失败、**主路径不可用**、**反常识默认**（如新建即已完成）、**一点改多对象**、主存读丢数据、功能 smoke `ok:false`、**计划必做意图被静默降级**（例：要求真实感商品/场景图 → 仅交几何 SVG 插画并以「非 placehold」过关）→ `result=fail` + blocking≥1
- **map**：台账/索引/L1 不同构 → map≥1（默认挡关账，走 closeout/回补）
- **residual（不挡 PASS）**：真书手点/30s 录像/截图未做、工作区未 commit、gitignore 卫生、**未引用的脚手架 CSS**、可选 polish、toast 时序闪一下但状态正确
- **禁止**把 residual 写成 blocking；仅 residual 时 **GATE result=pass**、blocking=0、residual=N
- **禁止**把「不可用 / 状态错 / 计划意图未达成」降成 residual 刷 PASS（先判能不能用、意图是否还在，再对勾选）
- **禁止**用更易 grep 的卫生项（死 CSS/未 commit）顶替用户可见缺口进 rework 队列

## 硬规则
1. **业务源码只读**；禁止为「刷绿」改应用代码
2. 只允许写入 `.cco-out/inspect/**`
3. 真阻塞遗漏 → VERDICT=FAIL + ISSUES；**可修的 blocking 写清 fix_wp**（须对准用户可见缺口；缺真图 → 写「搜索/下载图库或生成图落盘并改路径」），交给回补波补齐（不是甩给用户）
...
7. 结案证据优先用户可见层（预览主路径 / 关键图是否像计划意图）；**禁止**仅以「字符串扫描 / HTTP 200」单独证明体验类勾选
