...
planner/: Mode B plan job（job 写 proposed 前 inject_system_post · llm/heuristic 用 PLANNER_MAX_TASKS · digest 状态行优先 · critic_summary/chips/notes/critic_llm_used/cost/ms · 规则 critic + 可选 LLM 第二跳（设置或 env） · sanitize_proposed_deps · update_proposed_task(depends_on/**role/scope_paths S-role**) / remove_proposed_task · plan.user_edits.json + preserve_from_job_id（P2-1/P2-2） · **task_edit.rs** role/scope 补丁纯辅助 · LLM 解析 provider/role/scope/tags/outputs（P2-5） · 确认屏一键重拆/开巡检/开智能校对 · 真实 docs 金样 · 可选任务确认屏勾选 · **extract_work_phases 优先 #### A1/B2/U1-1 → W0/W1 → 波次**；失败则 **diagnose + recover 文档真标题**（禁止静默空壳）；仅无任何可恢复工作结构才 last-resort meta 四波（prompt 内写失败原因） · **CcoSplit SoT**（write_proposed→SQLite cco_split_*；load_proposed 优先 SoT→PlanIR；import plan.proposed；job_view 全字段 desk DTO） · **plan_mode=fast** 本地启发式 · 僵尸 planning kill pid + 5min hard timeout
