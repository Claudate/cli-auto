...
shell_print/: **多 CLI 共享 print 骨架** — scope 前缀 · stream_child · ShellPrintProvider · profiles（含 install_hint；**禁止** spawn 时 npm install；spawn stdin=null 防交互 CLI 挂 · codex 用 `--sandbox workspace-write` 代 deprecated `--full-auto`）· **平台输出契约（契约层 T1/T2）**：`profiles.rs` 每 CLI 挂 `result_kind`，`decode.rs` 按 kind 收编 log_events/stream_parse 散落的 stdout 解码，统一回答「怎么判成功 / 有没有执行动作证据 / 人话错误」→ `ports::WorkerOutcome`；`adapter.collect` 把 `done_marker/execution_evidence` 填进 `TaskResult`，调度层 `gates::enforce_success_evidence` 统一裁决（不再单信 exit）。**新增 CLI = 挂 profile + 一个 decoder，禁止再往 log_events/stream_parse 散落 if-branch**
