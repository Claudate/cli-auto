...
memory_store/: **P3 轻量语义记忆**（`~/.cco/memory/`）— mod.rs SQLite+embedding BLOB · tantivy BM25 · ONNX all-MiniLM-L6-v2（模型缺失回退 stub 零向量）· TTL/max_entries 归档 · port.rs LocalMemory（MemoryPort 适配 · 按次短开避免长占写锁）· tests.rs 单测
