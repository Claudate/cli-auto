...
preview/: **本地预览**（`detect.rs`：`package.json` scripts dev|start|preview → 否则根 `index.html` 用 `python3 -m http.server` · 独立进程组 · 探测端口后才报已启动 · stop/status · **不**经 Mode B）
...
settings.rs: get/set settings view（failover H4 · post_inspect/post_git_push/**post_open_pr** 系统收尾 · planner_critic_enabled 可选 LLM 第二跳 · **effort** 推理深度）
