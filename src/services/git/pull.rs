...
pub fn pull_with_git_config(
    ...
    match strategy {
        PullStrategy::Fail => {
            ...
            if !fetch_result.ok {
                bail!("fetch failed: {}", fetch_result.output.unwrap_or_default());
            }
...
pub fn push_with_conflict_resolution(
    ...
    branch: Option<&str>,
    force: bool,
    auto_pull_strategy: PullStrategy,
    ...
    match push(config, project, remote, branch, force) {
        ...
        Err(e) => {
            ...
            if !pull_result.ok {
                bail!(
                    "push rejected (non-fast-forward) and auto-pull failed: {}",
                    pull_result.message
                );
            }
            match push(config, project, remote, branch, force) {
...
fn resolve_remote_name(git: &GitConfig, project: &Path, explicit: Option<&str>) -> Result<String> {
    ...
    if let Some(r) = git.pick_push_remote(None) {
...
fn is_non_fast_forward(err: &str) -> bool {
    ...
    lower.contains("non-fast-forward")
        ...
        || lower.contains("the remote contains work")
}
...
fn execute_pull_rebase(project: &Path, remote: &str, branch: &str) -> Result<PullResult> {
    ...
    let out = Command::new("git")
        ...
        .output()
        .with_context(|| format!("git pull --rebase {remote} {branch}"))?;
    let stdout = String::from_utf8_lossy(&out.stdout).trim().to_string();
    let stderr = String::from_utf8_lossy(&out.stderr).trim().to_string();
    let combined = if stdout.is_empty() {
        stderr.clone()
    ...
    if !out.status.success() {
        bail!("git pull --rebase failed: {combined}");
    }
    let files = estimate_changed_files(&combined);
    Ok(PullResult {
        ok: true,
        message: format!("pulled {remote}/{branch} (rebase)"),
        files_changed: files,
...
fn execute_pull_merge(project: &Path, remote: &str, branch: &str) -> Result<PullResult> {
    ...
    let out = Command::new("git")
        ...
        .output()
        .with_context(|| format!("git pull --no-rebase {remote} {branch}"))?;
    let stdout = String::from_utf8_lossy(&out.stdout).trim().to_string();
    let stderr = String::from_utf8_lossy(&out.stderr).trim().to_string();
    let combined = if stdout.is_empty() {
        stderr.clone()
    ...
    if !out.status.success() {
        bail!("git pull (merge) failed: {combined}");
    }
    let files = estimate_changed_files(&combined);
    Ok(PullResult {
        ok: true,
        message: format!("pulled {remote}/{branch} (merge)"),
        files_changed: files,
        branch: current_branch,
        remote: remote.to_string(),
        merged: true,
...
mod tests {
    ...
    fn push_with_conflict_resolution_no_remote() {
        ...
        let result =
            push_with_conflict_resolution(&cfg, &root, None, None, false, PullStrategy::Fail);
