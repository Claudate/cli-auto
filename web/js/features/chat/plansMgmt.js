...
/**
 * W3: load latest plan jobs for each execution plan under a wave (best-effort).
 * @param {string} root
 * @param {Array} pool
 * @param {string} waveKey
 */
async function loadWaveJobsByPath(root, pool, waveKey) {
...
/**
 * W3-2: serially confirm each planned job in the wave (same confirm_start; no start_run).
 * Stops if a run is already active or confirm fails.
 */
export async function confirmWaveBatchSerial(waveKey) {
  ...
  for (let i = 0; i < ready.length; i++) {
    ...
    try {
      // Restore job into session so desk matches path
      ...
      // Give live a moment; next confirm waits for hasActiveRun clear ideally
      // but product default is one active run — stop batch after first success
...
/** W3: open next unsplit plan in the wave on the split desk. */
...
async function assignFromPlansMgmtPath(path) {
  if (!path) {
    toast("请先选中一份计划");
    return;
  }
