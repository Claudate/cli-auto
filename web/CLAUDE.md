...
css/: **tokens**（P4-1 三层 `--leaf-*`：static 色阶 → body alias → 暗色 `body[data-leaf-theme="dark"]` 覆写；legacy `--bg/--accent` 等为别名过渡；**组件禁写死颜色**）· **components**（P4-1 dsh 原语：`.dot.run/pending/danger` StateDot · `.leaf-card` · `.pill` · `.disclosure-row` · `.terminal-block`/`.diff-block`/`.read-block` · `.toast`；≤200 行 · 只引 alias/static）· layout · **select**（统一下拉 closed/open）· plan（含 split-route-advanced · **result-cost-chip**）· monitor（board-toolbar-side · is-result）· log · chat（**#chat-effort** 推理深度选择器）
