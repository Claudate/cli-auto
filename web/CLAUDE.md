...
js/shared/: **gateway.js**（IPC 唯一出口 · 含 **chatReadImageDataUrl** / **gitDoctor** 发布状态）· **store.js**（可订阅薄 store）· **statusUi.js**（D9 人话/badge/elapsed）· **markdown.js**（D9 确认屏/计划说明/**聊天气泡** md · **`![alt](path)` 本地图占位**）· **shellUi.js**（D9 pages/projects/run-lock · **B1 侧栏 × 移除项目** · **P4-2/8 侧栏 chrome**：折叠 56px rail · 搜索过滤 · 项目→计划树）· **sidebarPlans.js**（计划元数据缓存 · 默认五项 · 瞬态展开）· **clickOutside.js**（**B2** 展开 details/菜单/设置高级点空白收）· **confirmDialog.js**（应用内确认层 `confirmDialog()`→Promise&lt;bool&gt; · 替代 WKWebView 不可靠的 window.confirm/prompt · Esc/背板=取消 · danger 焦点在取消；main install `ccoConfirm`）· **selectUi.js**（原生 select 增强为 macOS 风下拉；保留 `.value`/`change`）· **icons.js**（Lucide 风格开源线条图标 · `data-icon` / `ccoIcon` · **禁止 emoji 作按钮图标**）· **workStyle.js**（方案 C 工作习惯四选一 · 可跳过 · 并发/模板/grain 种子 · 项目级覆盖 W4-2 · **不**改 plan_mode 默认 ai）· **thinkingOrb.js**（canvas 2D 思考 orb 渲染引擎：九状态点阵球 · 减弱动态效果时单帧静止 · IO 防泄漏；无 IPC · 纯展示）
...
P4-8：`shared/sidebarPlans.js` 以 gateway 计划元数据缓存渲染项目→计划二级树（默认五项、瞬态展开）；`ResultView` 将 `live.verification` 置入结果局部巡检列（约 320px，窄窗默认收起），不复制 DTO 或业务规则。
