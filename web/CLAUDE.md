...
js/features/settings/: **A5-2d ✅ · P-ship-D** settingsApi · settingsForm（**父级分区两栏**：左侧父级菜单 **Lucide 图标** → 滚动定位；**父区标题 sticky** 置顶；**settingsNav** 滚动高亮 + **syncPermissionNav** 授权拒写警示点）· Git · 版本发布 大区（**GitHub 发布状态** · 自动提交粒度 off/per_plan/per_task + push_after_commit + 自动开 PR）· doctorPage · shellBoot · uiActions · bindUi · **bindUiClick** · settingsNav（settings/doctor/gitDoctor/meta/open_monitor 经 gateway；事件表只绑意图）
