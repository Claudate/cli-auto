# 计划 01b · 职业路径画像（多维默认，按场景芯片与 delivery 推断）
...
> 问题：用户批评「职业 × 路径」被收成三档课或三字母——**没有按人群优化**。本文补齐。  
> 状态：决策向 · W0 文案/模板/默认旋钮真源  

[PROTOCOL]: **交付形态 trial/single/bundle（旧 L/M/H）= 引擎内部**；**禁止**空态三键教用户。职业优化 = **画像旋钮** + **场景芯片**。禁止每职业一套 App；禁止进门选职业。改默认先改本文与 [03](./03-path-empty-cta.md)。
...
| 错法 | 问题 |
|------|------|
| 职业只贴 L/M/H 三格 | 开场/CTA/验收全一样 → 没优化 |
| **空态逼用户选 L/M/H** | 又一门课；与「AI 更懂用户」相反（见 03 校正） |
| 职业锁死形态 | 小老板也能多页；PM 也能试一句 |

**正确结构**：

```text
场景芯片 / 对话 / 附件  →  套用 Persona 旋钮 + 推断 delivery（对内）
PersonaProfile           →  开场·CTA·闸·验收词…（用户可感差异）
delivery trial|single|bundle → 仅引擎（旧 L/M/H，不上主 UI）
P1–P6                    →  仅提示词透镜
```

用户**无感**字母档；只感「例子像我的事、按钮像下一步、验收像我的行话」。
...
| 旋钮 ID | 含义 | 取值例 |
|---------|------|--------|
| `delivery_bias` | 对内交付粒度偏向（**不上 UI**） | `trial` / `single` / `bundle`（可一主一副） |
...
用户当场：芯片 / 人话（「先看看」「多页一起」）
  > 本项目已存 persona / 场景
  > 画像 delivery_bias 等默认
  > 产品全局默认（single + soft 可跳过澄清）
```
...
| 旋钮 | 默认 |
|------|------|
| delivery_bias | **trial 主**，single 副（要完整说明时） |
...
| artifact | **demo_short** |
| primary_cta | **生成并看看**（trial）/ 写清楚再拆步（single） |
...
**与「只有短试」差在哪**：CTA、砍项、验收词、费用姿态都按「老板周末验证」调。
...
| 旋钮 | 默认 |
|------|------|
| delivery_bias | **single 主**，bundle 副（需求包/多 epic 时抬升） |
...
| 旋钮 | 默认 |
|------|------|
| delivery_bias | **single 主**，trial 副（只出一屏草稿时） |
...
| 旋钮 | 默认 |
|------|------|
| delivery_bias | **bundle 主**（多页/多语时），单活动页则 single |
...
| 旋钮 | 默认 |
|------|------|
| delivery_bias | **trial 主**，系列站 single |
...
| 旋钮 | 默认 |
|------|------|
| delivery_bias | **single 主**（单品完整上架），多 SKU/多活动 → bundle |
...
| 旋钮 | 默认 |
|------|------|
| delivery_bias | **single 主**（极少 trial；几乎不 bundle 除非多制度包） |
...
| 旋钮 | 默认 |
|------|------|
| delivery_bias | **single 主**，跨部门多线时 bundle |
...
| 旋钮 | 默认 |
|------|------|
| delivery_bias | single/bundle 随意；能力全开在 **高级** |
...
| 画像 | delivery 主/副 | 开场域 | 澄清 | 主 CTA | 直接执行 | 并行食欲 | 确认 | 验收词核心 |
|------|------------|--------|------|--------|----------|----------|------|------------|
| founder | trial / single | 客户可看 | none | 生成并看看 | offer | task | light | 演示 3 分钟 |
| pm | single / bundle | PRD/纪要 | soft2 | 拆成步骤/目录 | hide | task+plan | normal | 清单可勾 |
| designer | single / trial | 第一眼/主钮 | soft1 | 拆成可预览 | hide | task 谨慎 | normal | 预览像意图 |
| ops | **bundle** / single | 本波几页截止 | soft2 | 生成本波目录 | hide | **plan+task** | normal | 页开表收语齐 |
| creator | trial / single | 链接+语气 | none | 生成并看看 | offer | serial | light | 能发链接 |
| ecom | single / bundle | 卖点+禁编造 | soft2 | 拆成上架步骤 | danger | 多品分计划 | normal | **上架清单** |
| edu | single | 学员路径 | soft2 | 拆成开课步骤 | hide | serial | normal | 学员走通 |
| admin | single | 正式/必审 | soft2 | 草案等人确认 | **hide** | serial | **strict** | 存档未乱改 |
| biz_lead | single / bundle | 业务结果 | soft2 | 拆步/本波 | hide | task | normal | 可汇报 |

**结论**：九行在 **8+ 个旋钮** 上不同；不是三行交付课表。

---

## 4. 空态怎么用画像（仍 ≤3 概念 · **无三档键**）

**不要**：进门选职业；**不要**进门选快试/一份计划/多需求。  

**要**：

```text
主输入（第一）+ 占位随场景芯片变
芯片（可跳过）：验证想法 | 写清一件事 | 大促多页 | 上架 | 开课 | 发文 …
点芯片 → 套用 §2 全旋钮（含内部 delivery_bias）
系统也可从附件数/话术推断 delivery，不打扰用户
```

同屏 ≤3：**输入 · 例子 · 下一步**。芯片=业务例子，不是身份证、不是档位课。
...
| 用户此刻 | 合成 |
|----------|------|
| 电商 + 改主图文案 | 芯片上架 → ecom 旋钮；话短 → 内部 trial；验收仍「上架清单」 |
| PM + 周末试一句 | 话术「先看看」→ trial；pm 的 direct 仍 hide |
| 运营 + 单页 | 芯片或默认 single；opener 仍战役减配 |
| 行政 + 想快 | 可少问；**strict 确认 / hide direct 不可降** |

**硬规则**：用户人话可推 delivery；**不可**破 admin strict、全局 confirm。
