...
- **FAIL=0**（非 STRICT）
- **WARN≈9**（Rust 业务源 > soft 400 或 hard 600；**parse + normalize hard 已消**）
- web S8 classic facade 已出 GIANTS；`state.js` ~248 在 soft 内

## Rust WARN 列表（2026-08-11 更新 · parse/normalize 已拆）

| 文件 | 约行 | 级 |
|------|------|-----|
| `src/domain/inspect/parse/issues.rs` | ~488 | soft |
| ~~`src/domain/inspect/parse.rs`~~ | ~~869→拆~~ | ✅ hard 已消（verdict/issues + tests 分文件） |
| ~~`src/domain/chat/normalize.rs`~~ | ~~800→拆~~ | ✅ hard 已消（plan_md / acceptance / verification；均 ≤ soft） |
...
4. ✅ **已完成**：`inspect/parse` 按纯函数边界拆（verdict / issues / issues_tests；hard 已消）
5. ✅ **已完成**：`chat/normalize` 按纯函数边界拆（plan_md / acceptance / verification；hard 已消 · 均 ≤ soft）
6. 下刀 hard 残余：`clarify.rs` · `plan/materialize.rs`；`STRICT=1` 仅在 CI 对**新增**hard 违约失败（若要做须改 `check-arch.sh` + L1）
