...
> 状态：**D0–D4 已闭环**；**D5 backlog 池 t15 已建立（不排期则不碰）**；**§5 推荐执行顺序 t16 已冻结**；**§5.4 任务量与 Agent 策略 t17 已冻结**；**§6 成功标准 t18 已冻结（五指标全绿）**；**§7 非目标 t19 已冻结**；本文件为未完善唯一总账  
> 日期：2026-07-18  
...
> **注意**：本节 = **产品/文档默认假设**（历史执行前问卷）。  
> **Agent 调度策略真源** = **§5.4（t17）**，原称「对应 AGENTS §8」——**不是**本节。二者勿混。  
> 下列默认在 D0–D4 中已落地；**新**默认变更仍须用户决议（§5.4.3），不可 Agent 静默改。

请确认以下默认假设（回「按默认」或逐条改）：

1. **规范根**：以 `docs/` + 根 `CLAUDE.md` 为 GEB 真源，**不**新建平行 `.md/` 目录（避免双份）。✅ D0  
2. **桌面默认**：保持 **分配后自动开跑**；高级区加「规划后确认」开关。✅ D1  
3. **下一执行阶段**：先做 **D0 文档同构**，再进入 D1 决议实现。✅ 主序已走完 D0–D4；后续 = D5 出池或热改  
4. **D4 大拆分**：✅ t14 已按切分地图物理落地；后续仅热改时再切残差。  
