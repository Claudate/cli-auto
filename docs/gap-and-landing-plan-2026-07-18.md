...
| ID | 缺口 | 状态 | 来源 / 锚点 | 备注 |
|----|------|------|-------------|------|
| P2-1 | 确认屏删任务 / 改依赖 | ✅ **已落地** | Mode B2 · `remove_proposed_task` / `depends_on` | **t30**；确认屏删除钮 + 依赖勾选 |
...
| P2-7 | M5：SDK provider / Mermaid / 自动开 PR / Windows launcher | ⚠ **t41+t59+t60+t62+t65+t67 部分** | orchestrator M5 · `format_mermaid` · `terminal/external` · `provider/sdk` · `system_post` open-pr | **Mermaid ✅ t41**；**Windows launcher ✅ t59**；**SDK S0 ✅ t60**；**自动开 PR ✅ t62（S-PR · 默认关 · gh）**；**SDK S1 HTTP ✅ t65**；**SDK S2 tool loop ✅ t67** |
...
| 无 Codex provider | §1.3 Providers；**P2-8 ✅ t35**（文档债扫尾） |
| 无 `cco plan` / `run` 不规划 | **已有** `cco plan`；`run` 散文 plan job / 结构化 skip（P0-1 **D1 闭环**） |
| 无计划时不能在 App 内起草 | **P-chat ✅ C0–C2**：桌面聊天 → 落盘 → 分配；C3 **t32–t34 全闭环**（多会话·方案 B·diff·流式）→**P2-9 ✅** · 体验→**P2-10 ✅** |
...
|-------|------|------------|----------|----------------------|
| **P2-1** | 确认屏删任务 / 改依赖 | Mode B2 可选 | **已落地**（t30）：`remove_proposed_task` · `update_proposed_task(depends_on)` · 确认屏 UI | — |
...
| **P2-7** | M5：SDK provider / Mermaid / 自动 PR / Windows launcher | orchestrator M5 | ⚠ **t41+t59+t60+t62+t65+t67**：Mermaid ✅；Windows ✅；**SDK S0 ✅**；**自动开 PR ✅ t62（S-PR）**；**SDK S1 HTTP ✅ t65**；**SDK S2 tool loop ✅ t67** | 勿整包；下一项须再点名 |
...
- [x] 明确 **禁止在 D5 任务内实现**（未出池项）：portable-pty 真交互写；**已出池落地**：P2-1…6/8–12 等见上表；**P2-5 伪 PTY 网格 t40**；**P2-7 部分**：Mermaid t41 · Windows launcher t59 · **SDK S0 t60** · **自动开 PR t62（S-PR · 默认关 · gh）** · **SDK S1 HTTP t65** · **SDK S2 tool loop t67**；真 PTY 仍池内  
...
| 2026-07-20 | **t31 / multi-cli P2 残差 + 监视 P2-3 主项**：`write_task_diff` CHANGED.md（P2-2）；live `handoff_board` + 打开账本 + 确认屏引擎可见（P2-6）；日志事件过滤/轻量 ANSI/导出 MD（terminal P2-3）；handoff 13 测绿；P2-4 系统多窗口 / P2-5 真 PTY / multi-cli P2-4·P2-5 仍 D5；**不**回灌 D0–D4 |
| 2026-07-20 | **t32 / P2-9 C3 多会话部分落地**：`chat_list_sessions`/`chat_new_session`/`chat_delete_session` + Tauri 薄壳 + 聊天页会话下拉/新建/删除；缓存键 `project::session_id`；chat 27 测绿；流式 / 方案 B / 计划 diff 仍残差；**不**回灌 D0–D4 / 不改 Q2 默认方案 A |
...
| 2026-07-20 | **t42 / P2-16 立项（PRODUCT 主路径轻量化）**：方向真源 [`../PRODUCT.md`](../PRODUCT.md)；优化计划 [`product-mainpath-optimize-2026-07-20.md`](./product-mainpath-optimize-2026-07-20.md) O0 叙事 · O1 结果摘要 · O2 模板；§2.1/§4 D5 增 **P2-16 / P-product-light**（池内可出池）；**不**实现代码；**不**回灌 D0–D4 / 已 ✅ P2；**不**出池 P2-7 剩余 |
| 2026-07-20 | **t43 / P2-16 升格主路径大改**：UI/操作流重做；**计划拆分 S0–S16（含开源借鉴） 第一优先**；默认停拆分台（高级才 auto-start）；任务组 S/F/R/V/T/X；**不**换框架；**不**实现代码本条 |
| 2026-07-20 | **t44 / P2-16 开源拆计划借鉴入库**：对照 OpenHands/Aider/MetaGPT/CrewAI/AutoGen/LangGraph/planning-with-files；计划 §2.5 + R-S10–17 + 任务 S12–S16；**只借模式不换运行时**；**不**实现代码本条 |
| 2026-07-20 | **t45 / P2-17 架构大改立项**：增 [`architecture-redesign-2026-07-20.md`](./architecture-redesign-2026-07-20.md)（六边形+用例 · MVVM · A0–A5）；§2.1/§4 增 **P2-17 / P-arch-redesign** 出池实施；P2-16 降为交互参考；L1/L2/PRODUCT 指针；**不**实现代码本条；**不**回灌 D0–D4 |
| 2026-07-20 | **t46 / P2-16 波次 5 闭环**：`web/js/templates.js`（出海落地页+通用需求大纲一键落盘 · 欢迎/聊天入口）· S14 `btn-split-writeback` 步骤摘要写回（默认关、不覆盖正文）· S16 帮助 R-S10…17 清单 · package-app X3 目视标记 · node --check + cargo test；§2/§4 P2-16 ✅ UI；**架构边界仍 P2-17**；**不**换栈/旁路 confirm_start；**不**回灌 D0–D4 |
| 2026-07-20 | **t47 / P2-17 A0 基线完成**：分支 `feat/arch-a0-baseline`；`docs/contracts/`（behavior-golden · run-dir · plan-job）；`tests/a0_behavior_golden.rs` 锁 confirm/stop-Pending/soft-fill/optional；`src/{domain,app,ports}` 空骨架可编译；架构计划 §7/§11 A0 勾满；**无产品行为变化**；**不**启动 A1 巨石迁移；**不**回灌 D0–D4 / 旁路 confirm_start |
| 2026-07-20 | **t48 / P2-17 A1 首刀**：分支 `feat/arch-a1-backend`；`domain/plan`（types/validate/materialize/optional/routing/system_ids）；`plan/mod.rs` facade ~232 行 + `plan_tests.rs`；`app/split::confirm` 唯一开跑、`services::confirm_start` 委托；A0 金样+lib 测绿；**未**切 scheduler/handoff/chat；**不**旁路 confirm；**不**回灌 D0–D4 |
| 2026-07-20 | **t49 / P2-17 A1-3 run 状态机 + 薄编排**：`domain/run`（status/retry/active 纯规则）；`runtime/scheduler` 拆多文件（mod/tick/finish/start/patrol/gates/active/types，单文件 ≤600）；`app/run` 用例面；scheduler 出巨石榜；A0+scheduler_fake+retry_and_stall+mixed+mode_b 绿；**未**做 A1-4 WorkerPort / A1-5 handoff 切 VERDICT / A1-6 chat；**不**旁路 confirm；**不**改 run.json schema；**不**回灌 D0–D4 |
| 2026-07-20 | **t50 / P2-17 A1-4 WorkerPort + 策略对象**：`ports::WorkerPort`（start/poll/stop/collect/preflight/capabilities + DTO）；`domain/worker`（ProviderId/Route/Capability · soft-fill Soft/Force · FailoverPolicy · IsolationOnFail）；claude/codex/fake 实现 port（`WorkerProvider` 别名）；registry 按 port 装配；scheduler 经 port + 策略；CLI/planner soft-fill 委托 domain；A0+mixed+retry+scheduler_fake+mode_b 绿；**未**做 A1-5 handoff VERDICT / A1-6 chat / A1-7 Tauri 全量委托；**不**旁路 confirm；**不**改 run.json schema；**不**回灌 D0–D4 |
| 2026-07-20 | **t51 / P2-17 A1-5 Handoff/Inspect 分离**：`domain/inspect`（parse_verdict/issues · gate 决策 · REWORK_MAX/MAP 常量）；`ports::HandoffStore`；`runtime/handoff/*` 多文件适配器（model/paths/inspect_io/lifecycle/prefix/rework/store/tests，单文件 ≤600）；scheduler/gates 经 facade + `inspect_gate_fail_reason`（**零** VERDICT 正文解析）；handoff 出巨石榜；A0+scheduler_fake+handoff_ledger+mixed+mode_b+retry 绿；**未**做 A1-6 chat / A1-7 Tauri；**不**旁路 confirm；**不**改 handoff.json/run.json schema；**不**回灌 D0–D4 |
| 2026-07-20 | **t52 / P2-17 A1-6 Chat 切开 + app/chat**：`domain/chat`（extract_plan_fence/title/normalize/stream_parse/id/text 纯规则 + 测）；`app/chat` 用例面（session/send/stream/save_plan/read_plan_md/normalize/attachment/cleanup；**禁止** confirm/start_run）；`services/chat/*` 多文件 IO（types/session/send/stream/plan_md/attachment/cli_call/normalize/paths/tests，单文件 ≤600）；services re-export 保持桌面/Tauri 可编译；chat 出巨石榜；A0+mode_b+lib chat 绿；**未**建 ChatStore trait；**未**做 A1-7 Tauri 全量委托 / A2 前端 MVVM；**不**改 session JSON / `.cco/chat/` 路径；**不**旁路 confirm；**不**回灌 D0–D4 |
| 2026-07-21 | **t53 / P2-17 A1-7 Tauri/CLI 全量委托 app**：`src-tauri/src/lib.rs` chat/split/run/plan-job/stop/resume/rework → `cco::app::{chat,split,run}`；CLI `stop`/`plan`/`run --provider` soft-fill 经 app；`app/run` 扩 list/load/stop/resume/rework/soft-fill 表面；`services` 标 deprecated facade（未删）；IPC 命令名与 DTO 字段兼容；A0+mode_b+lib 绿；check-arch 仅 web 巨石 WARN；**未**做 A2 MVVM / 删 services / 改 web 业务；**不**旁路 confirm；**不**回灌 D0–D4 |
| 2026-07-21 | **t54 / P2-17 A2 前端 MVVM 骨架**：分支 `feat/arch-a2-frontend`；`web/js/main.js`（type=module）· `shared/{gateway,store}.js` · `app/{AppViewModel,routes}.js` · `features/chat/*`（listSessions/send/savePlan 经 gateway）；index 挂 module；tokens 主 CTA；chat.js 桥接 ccoChat；**不**改 IPC 名/DTO · **不**动 Rust app 语义 · **不**重写 plan.js · **不**旁路 confirm；A0+mode_b+lib 绿；check-arch features 无散落 invoke；巨石 chat/plan 仍 WARN；A3 拆分台待；**不**回灌 D0–D4 |
| 2026-07-21 | **t55 / P2-17 A3 拆分台垂直切片**：分支 `feat/arch-a3-split-desk`；`web/js/features/split/*`（splitApi·SplitViewModel·splitRender·splitDetail·SplitView）· `window.ccoSplit`；plan.js confirm 委托 feature（只抽/一行委托）；三栏波次/卡片/详情；高级路由折叠（provider 经 gateway；role/scope 展示）；optional 勾选；replan `preserve_from_job_id`；`confirmStart`→`goRun` 唯一开跑；**零** Rust 语义 diff；**不**旁路 confirm / **不**改 soft-fill；A0+mode_b+lib 绿；check-arch features 无散落 invoke；A4 结果台待；**不**回灌 D0–D4 |
| 2026-07-21 | **t56 / P2-17 A4 执行台+结果台+Inspect**：同分支续 A3 tip（未另开 feat/arch-a4，文档写清 A4 起点）；`web/js/features/run/*`（进度·stall·stop/resume/stopTask·logPanel）· `features/result/*`（摘要·回补·accept residual·inspectCopy 人话）；`window.ccoRun`/`ccoResult`；monitor/result/log/doctor 委托；终态 `goResult`；回补 `start_rework`→`goRun`（非 confirm/`start_run`）；**零** Rust 语义 diff；A0+mode_b+lib 绿；check-arch features 无散落 invoke；A5 删巨石待；**不**回灌 D0–D4 |
...
| 2026-07-21 | **t67 / P2-7 单项 SDK S2 tool loop（勿整包 M5）**：`runtime/provider/sdk_tool_loop/` `AnthropicToolLoopBackend` 实现 `SdkBackend`（multi-turn `/v1/messages` + tools · cwd-scoped `read_file`/`list_dir`/`write_file` · mockable `MessagesHttpClient`）；`build_sdk_provider` 按 `bin=tools`/`CCO_SDK_BACKEND=tools` 注入；`CCO_SDK_MAX_TOOL_ROUNDS`；registry 仍 **opt-in**（`providers.sdk.enabled` 默认 false）；S0/S1 默认不变；单测 multi-round mock + path escape + write + max rounds；**不**做 Bash/真 PTY/自动 PR 返工/旁路 confirm；**不**回灌 D0–D4 |
