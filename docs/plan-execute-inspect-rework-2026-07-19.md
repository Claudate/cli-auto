...
| 级别 | 定义 | 默认处置 |
|------|------|----------|
| **blocking** | 计划必做未完成；**验收/意图被静默降级**（例：真实感商品图 → 仅几何 SVG 并以「非 placehold」过关）；回归红；越界改了非目标 | **必须** 回补波或用户显式「接受残留」；fix_wp 须对准用户可见缺口（缺图 → 搜图落盘） |
| **map** | L1/L2/总账/计划状态指针与 § 勾选不同构 | **默认 blocking**（GEB 法则）；回补可只改文档 |
| **residual** | 可选 F2、不排期项、计划允许的后续、手点/录像/未 commit、**未引用脚手架 CSS** | 不阻塞 PASS；写入 ISSUES 附录；**不得**用 residual 吞掉计划主标准 |
