...
loopx-longtask-borrow-2026-08-22.md: **LoopX 借鉴 · 长任务治理优化**（LoopX=骑在 harness 上的控制平面；对照证 Leaf 内核方向对 · **LX1** should-run 决策收敛 `domain/run/tick_decision`(薄编排 规则8) · **LX2** pending_user_gate 人话对象（`domain/run/pending_gate` + PlanJobView 填 + 拆分台「等你回答：X」渲染） · **LX3** safe-fallback 登记不排期 · 拒抄 headless常驻/peer-lease；**LX0/LX1-a/LX1-b/LX2 ✅ 2026-08-22 · 本借鉴唯一勾选**；上位 harness-inspired-roadmap §A）
...
session-digest-event-2026-08-14.md: **B3 Session Digest 事件化**（B1 doc ✅ · B2 `session.rs::append_session_event` 会话级事件日志 ✅ · B3 `send.rs` 压缩成立写 `context_compressed`（chars_before/after + sha256 digest_hash）✅ · B4 测试 + L2/索引 + check-arch STRICT=1 ✅；**本能力唯一勾选**；chat 相非 run 相 · 诊断默认关 · 不改 ChatSendResponse）
