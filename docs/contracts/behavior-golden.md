...
| **A0-R4** | **optional 不可静默 auto-start** | `optional && !include` 在 confirm 的 `materialize_selected_tasks` 中丢弃；全不选报错；不得在无确认勾选时塞进 run | unit：`materialize_drops_unselected_optional` · `a0_materialize_rejects_all_optional_unselected` · 集成：`a0_optional_unselected_not_in_run_after_confirm` | ✅ |
